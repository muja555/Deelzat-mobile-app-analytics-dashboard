...
class Accordion extends React.Component {
    ...
    render() {
        ...
        const nodes = React.Children.map(this.props.children, (child, index) => (
            <div key={index}
                 ref={div => { this.nodes[index] = { ref: div } }}
                 className={this.state.index === index ? this.props.openClassName : ''}
                style={{marginBottom: 20}}>
                <div style={{ flex: 1, justifyContent: 'center', alignItems:"center", }} onClick={() => this.toggle(index, true)} className={'accordion-title'}>{child.props['data-header']}</div>
                <div style={{ ...style, height: this.getHeight(index), display: "flex", alignItems: 'center', width: '100%', textAlign: 'center' }}>{child}</div>
            </div>
        ))
        return <div className={this.props.className} style={{width: '100%'}}>{nodes}</div>
    }
}
